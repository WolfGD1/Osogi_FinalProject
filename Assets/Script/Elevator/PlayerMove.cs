...
public class PlayerMove : MonoBehaviour
{
    ...
    public GameObject GhostPicture;
    float alpha = 0;
    public Image Fade;
    public Text t;
    int cnt = 0;
    ...
    void HorrorAct()
    {
        ...
        if (MM.i == 44 && is5thFloor)
        {
            //Horror.color = new Color(1, 1, 1, 1);
            if ((Mouse_Horizontal % 360 > 90 || Mouse_Horizontal % 360 < -90) && !MView.MemoView)
            {
                transform.position = new Vector3(0, 16, 30);
                Mouse_Horizontal = 180;
                Mouse_Vertical = 0;
                t.text = "...";
                StartCoroutine("End7");
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.gameObject.transform.name == "EXIT" && MM.i != 444 && MM.i != 44 && MM.Fail == false && MM.End == 1)
        {
            MM.Fail = true;
            MView.MemoView = false;
            t.text = "뭔가 느낌이 이상하다...";
        ...
        else if (other.gameObject.transform.name == "EXIT" && MM.i == 44)
        {
            MView.MemoView = false;
            t.text = "뒤에서 한기가 느껴진다...";
            GhostPicture.SetActive(true);
            is5thFloor = true;
            //StartCoroutine("FadeIn");
        }
        ...
        else if (other.gameObject.transform.name == "EXIT2")
        {
            //MM.Fail = true;
            t.text = "한기가 사라졌다. 원래 세상으로 돌아온 듯하다.";
            StartCoroutine("End6");
        }
    }
    ...
    IEnumerator End6()
    {
        ...
        //Fade.color = new Color(0, 0.17f, 0.44f, 1);
        yield return new WaitForSeconds(1f);
        SceneManager.LoadScene("End0-6"); //탈출하는 엔딩
    }
    IEnumerator End7()
    {
        var MM = Mouse.GetComponent<Mouse>();
        ...
        //Fade.color = new Color(0, 0.17f, 0.44f, 1);
        yield return new WaitForSeconds(1f);
        SceneManager.LoadScene("End0-7"); //뒤돌아보는 엔딩
    }
}
